...
public class Shell {
    ...
    public static void sort(Comparable[] a) {
        ...
        int increament = 1;
        while (increament < length/3) increament = 3*increament + 1;
        //int increament = length / 3;

        while(increament >= 1){
            for(int i = increament;i < length; ++i){
                for(int j = i; j >=increament && less(a[j], a[j-increament]); j-= increament){
                    exch(a, j, j-increament);
                }
            }
            assert isHsorted(a, increament);
            increament /= 3;
        }
        assert isSorted(a);
    }
